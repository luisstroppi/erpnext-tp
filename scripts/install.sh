...
TEMP_REDIS_QUEUE=false
TEMP_REDIS_CACHE=false

cleanup_bench_redis() {
  if [[ "$TEMP_REDIS_QUEUE" == true ]]; then
    redis-cli -p 11000 shutdown >/dev/null 2>&1 || true
  fi
  if [[ "$TEMP_REDIS_CACHE" == true ]]; then
    redis-cli -p 13000 shutdown >/dev/null 2>&1 || true
  fi
}

trap cleanup_bench_redis EXIT
...
log "4/10 - Verifying Redis binary"
...
start_temp_redis() {
  ...
  [[ -f "$config_file" ]] || die "Missing Bench Redis config: $config_file"
  redis-server "$config_file" --daemonize yes

  for _ in {1..20}; do
    if redis-cli -p "$port" ping 2>/dev/null | grep -q PONG; then
      printf -v "$flag_name" '%s' true
      ok "$label started temporarily on port $port"
      return
    fi
    sleep 0.25
  done

  die "$label did not start on port $port."
}

# ERPNext installation can enqueue jobs before `bench start` is running.
# Start the Redis instances generated by Bench temporarily; the EXIT trap
# stops only the instances started by this installer so later `bench start`
# can manage them normally through the Procfile.
start_temp_redis "Redis Queue" 11000 "config/redis_queue.conf" TEMP_REDIS_QUEUE
start_temp_redis "Redis Cache" 13000 "config/redis_cache.conf" TEMP_REDIS_CACHE
